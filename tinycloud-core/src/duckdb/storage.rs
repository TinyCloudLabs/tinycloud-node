...
fn apply_security_settings(conn: &Connection, max_memory: &str) -> Result<(), DuckDbError> {
...
pub fn promote_to_file(
    ...
    // Use EXPORT DATABASE to a temp dir, then IMPORT into the file-backed connection
    let temp_dir = tempfile::tempdir().map_err(|e| DuckDbError::Internal(e.to_string()))?;
    let temp_path = temp_dir.path().to_string_lossy().to_string();

    conn.execute_batch(&format!("EXPORT DATABASE '{}';", temp_path))
