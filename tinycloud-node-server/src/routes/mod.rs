...
mod tests {
    ...
    #[tokio::test]
    async fn w5_policy_runtime_issued_grant_allows_native_read_then_cutoff_denies() -> Result<()> {
        use chrono::{DateTime, TimeZone, Utc};
        use ed25519_dalek::{Signer, SigningKey};
        use policy_core::{
            requested_capabilities_hash_hex, Audit, AuditIssuance, DelegationMode,
            DenialDisclosure, Disclosure, EvidenceAuthority, EvidenceExpression, EvidenceFreshness,
            EvidenceRequirement, GrantChallenge, GrantOutput, GrantPresentation, GrantTemplate,
            HolderBindingProof, HolderEnrollment, HolderEnrollmentDisposition,
            HolderEnrollmentStatus, Policy, PolicyCapability, PolicyDisposition, PolicyResource,
            PolicyStatus, PresentedEvidence, RevocationMode, Signature, SignatureSuite,
        };
        use policy_runtime::{
            EvidenceSatisfaction, EvidenceVerifier, GrantIssueRequest, GrantIssuer, PolicyRuntime,
            PolicySpaceState, PortableDelegation, RuntimeConfig, RuntimeError,
        ...
        impl RuntimeNodeGrantIssuer {
            fn new(issuer_did: String, delegation_hash: tinycloud_core::hash::Hash) -> Self {
                Self {
                    issuer_did,
                    delegation_hash,
                    issued: None,
        ...
        fn signed_presentation(
            ...
            presentation.holder_signature = Signature {
                ...
                value: base64::encode_config(
                    holder_key.sign(&digest).to_bytes(),
        ...
        state.insert_policy(policy(&space));
        state.insert_policy_status(active_status(&space))?;
        ...
        let presentation = signed_presentation(&space, &challenge, &holder_did, &holder_key);
        ...
        let issued_request = runtime
            ...
            .issued
            .as_ref()
            .expect("runtime must issue one grant")
            .clone();
        ...
        let constrained_caveat = policy_capability(&space)
            .caveats
            .expect("policy capability must carry SQL caveat");
        let mut invocation_caps = Capabilities::new();
        let mut invocation_nb = std::collections::BTreeMap::new();
        for (key, value) in constrained_caveat
            .as_object()
