...
fn public_api_from_config(config_path: impl AsRef<Path>) -> PublicApi {
    ...
    let cfg = if config_path.exists() {
        let figment = rocket::figment::Figment::from(rocket::Config::default())
            ...
            .merge(rocket::figment::providers::Toml::file(config_path).nested());
        figment.extract::<config::Config>().ok().map(|mut cfg| {
            cfg.storage.resolve();
            cfg
        })
    } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn macos_paths_use_override_root() {
        let _lock = env_lock();
    ...
    #[tokio::test]
    async fn linux_user_paths_respect_xdg() {
        let _lock = env_lock();
    ...
    #[tokio::test]
    async fn status_json_shape_serializes_expected_fields() {
