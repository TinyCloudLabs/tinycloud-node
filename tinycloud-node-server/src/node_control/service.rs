...
fn identity_source_warnings(paths: &ProfilePaths, control_config: Option<&Value>) -> Vec<String> {
    let mut warnings = Vec::new();
    let static_source = match control_config {
        Some(config) => matches!(
            config
                .get("config")
...
fn discover_control_session_for_paths(paths: &ProfilePaths) -> Result<ControlSession> {
    let _manifest = load_service_manifest_if_present(&paths.service_manifest_path)?
...
fn public_api_from_config(config_path: impl AsRef<Path>) -> PublicApi {
    ...
    let rocket_config = match rocket::Config::try_from(figment.clone()) {
        ...
        Err(err) => {
            eprintln!(
                "warning: failed to parse effective server config ({err}); reporting default public API bind"
            );
...
"#,
    ...
    fn bootstrap_config_parses_through_rocket_config() {
        ...
        // The bootstrap config must be bootable from Rocket's perspective: it has to
        // survive the same figment + rocket::Config::try_from path used by
